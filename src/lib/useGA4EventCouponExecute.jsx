...
const useGA4EventCouponExecute = (couponCode) => {
  let script = null;
  ...
  useEffect(() => {
    if (process.env.REACT_APP_ENVIRONMENT === 'cardel-product' && couponCode && couponCode != '') {
      TagManager.initialize({gtmId: 'GTM-NG8K5TRD'});
      TagManager.dataLayer({
        dataLayer: {
          event: 'custom_page_view',
          "eventName": "coupon_execute",
    ...
    else if ((process.env.REACT_APP_ENVIRONMENT === 'cardel-develop') && couponCode && couponCode != '') {
...
export default useGA4EventCouponExecute;