...
export default function useUndercard(props) {
    ...
    //  useEffect終了処理
    useEffect(() => {
        console.log("[useUndercard]isVideoEndedを検知==>", isVideoEnded)
        if(isVideoEnded){
    ...
    //  （４）選出されたテーマの中でSTEPの抽選
    useEffect(() => {
        console.log("[useUndercard]開始のタイミングで１回だけ設定処理isUndercardStarted==>", isUndercardStarted)
        //  開始のタイミングで１回だけ設定処理
    ...
    //  isUndercardAppearがtrueに変化した時

            let UndercardTick = () => {
                console.log("[useUndercard]UndercardTick = () =>")
                if(UndercardTicker < UndercardDuration){
                    //  制限時間内 + 演出がある + ある場合で表示当選
                    console.log("[useUndercard]UndercardTick = () =>制限時間内 + 演出がある + ある場合で表示当選",UndercardTicker)
                ...
                ////////////////////////////////////
                    ...
                    console.log("[useUndercard]UndercardTick = () =>終了==>", UndercardTicker)
    ...
    // useEffect(() => {
        ...
        }else if(isUndercardStarted  && !isShowUndercard){
            console.log("[useUndercard]Undercardが開始中 + まだ非表示 + 始動前演出がある + 抽選の結果表示許可")
        // //  Undercardが開始中 + まだ非表示 + 始動前演出がある + 抽選の結果表示許可 => showフラグを立てる
            if(!isShowUndercard){setIsShowUndercard(true)}
            //  一応クリアする
            clearInterval(UndercardInterval);
        }else{
            console.log("[useUndercard]setInterval(UndercardTick,200)")
            // intervalがすでに有るのなら、それはキャンセル。
            if(UndercardInterval) {
                clearInterval(UndercardInterval);
            }
            // あらためてintervalを作成
            UndercardInterval = setInterval(UndercardTick,200);
        }
    // }, [isUndercardEnded, isUndercardStarted, isShowUndercard, hasUndercard, isUndercardAppear]);


    console.log("[useUndercard]UndercardTicker：", UndercardTicker,{
