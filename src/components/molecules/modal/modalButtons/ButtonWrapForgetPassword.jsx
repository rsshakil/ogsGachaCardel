...
export const ButtonWrapForgetPassword = ({handleOnSubmit}) => {
    ...
    async function doChangePassword(e) {
        const formElements = document.getElementById('passwordUpdateFrom').elements;
        let formData ={};
        ...
        const { token, newPassword1, newPassword2 } = formData;

        let tokenErrorMessage = null;
        if (!token) {
            tokenErrorMessage = 'Please_enter_your_token';
        }
        ...
        if (!token || !newPassword1 || newPassword1.length<6 || newPassword1.length>32 || !newPassword2 || (newPassword1 !== newPassword2) ) return;
        ...
        try {
            ...
            if (response.status == 200) {
                ...
                setModalState((prevState) => ({
                    ...
                    modalType: 'error',
                    mode: "",
                    data: {title: "",body: intl.formatMessage({ id: 'A_system_error_has_occurred__Please_try_again' }),backToPrevModal:true,prevModalName:"ForgetPassword"}
                }))
            }
        } catch (err) {
            // console.log("err", err);
            ...
            if (err.response) {
                ...
                if(statusCode==101){//user_id_invalid_error
                    setModalState((prevState) => ({
                        ...
                        modalType: 'error',
                        mode: "",
                        data: {title: "",body: intl.formatMessage({ id: 'forget_password_token_verification_error' }),backToPrevModal:true,prevModalName:"ForgetPassword"}
                    }))
                }else if(statusCode==102){//Token expired
                    setModalState((prevState) => ({
                        ...prevState,
                        BaseModalOpen: true,
                        modalType: 'error',
                        mode: "",
                        data: {title: "",body: intl.formatMessage({ id: 'forget_password_token_expired_error' }),backToPrevModal:true,prevModalName:"ForgetPassword"}
                    }))
                    ...
                    setModalState((prevState) => ({
                        ...
                        modalType: 'error',
                        mode: "",
                        data: {title: "",body: intl.formatMessage({ id: 'new_password_conf_password_not_match_error' }),backToPrevModal:true,prevModalName:"ForgetPassword"}
                    }))
                    ...
                    setModalState((prevState) => ({
                        ...
                        modalType: 'error',
                        mode: "",
                        data: {title: "",body: intl.formatMessage({ id: 'old_password_same_as_new_error' }),backToPrevModal:true,prevModalName:"ForgetPassword"}
                    }))
                }else{
                    setModalState((prevState) => ({
                        ...prevState,
