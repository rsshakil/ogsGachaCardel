...
export const ContentStripe = () => {
    ...
    useUpdateEffect(() => {
        ...
        if(modalStateValue.modalType === 'Stripe' && isTimeout){
            clearTimeout(testTimer)
    ...
    useLayoutEffect(() => {
        ...
        testTimer = setTimeout(() => {
            ...
            //  タイムアウト後の情報が古いので（カント開始時）フラグだけ立てて外側で鮮度高い情報で処理する
            setIsTimeout(true)
            //  タイムアウトの時間　通常30秒:30000
        }, 30000);
        //  利用不能の時のタイムアウト離脱
        ////////////////////////////////
    ...
    useEffectOnce(() => {
        ...
        if(localStorage.getItem("token")){
            ...
            fetch(queries.paymentURL + queries.createPayment + "?cpp=" + modalStateValue.data.chargePoint + "&token=" + localStorage.getItem("token") + "&browserUUID=" + browserUUID + "&browserUserAgent=" + encodeURIComponent(browserUserAgent) + "&appRendersDateTime=" + Math.floor( appRendersDateTime / 1000 ), {
                method: 'get',
                headers: headersParam(),
                credentials: 'include',
                mode: 'cors',
	...
	}, [modalStateValue.data.PaymentElementReady]);
    //  stripePromise clientSecret PaymentElementReady==>３種の神器チェック
    ////////////////////////////////
                        ...
                        options={{ 
                            ...
                            appearance: {
                                ...
                                variables: {
                                  ...
                                  colorSuccess: '#00ff00',
                                  colorTextSecondary: '#0000FF',
                                  iconChevronDownColor : '#0000ff',
