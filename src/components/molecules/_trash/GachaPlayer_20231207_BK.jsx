...
export const GachaPlayer = () => {
    ...
    //  北斗の状態のカスタムhookチェック❗️廃止予定
    // const checkHokuto = useHokuto({
    //     currentTime : VideoRefCurrent.currentTime,
    //     ShowHokuto : ShowHokuto,
    //     isVideoEnded: isVideoEnded,
    // });

    //  Undercard状態のカスタムhookチェック
    const checkUndercard = useUndercard({
        currentTime : VideoRefCurrent.currentTime,
        currentEnded : videoRef.current?.ended,
        ShowUndercard : ShowUndercard,
        isVideoEnded: isVideoEnded,
    ...
    ////////////////////////////////////////////////////////
    //  BufferedRateの変化
    //  北斗中でなくボタン中でなければ積極的に目覚めさせる
    useEffect(() => {
        videoLog({before : 'useEffect [BufferedRate]動画読み込み割合', after : BufferedRate})
        //  終了でなく　北斗中でなくボタン中でなければ積極的に目覚めさせる
        //  通常はBufferedRate > 33であるが少し甘くする
        if(!isVideoEnded && !ShowUndercard && !checkButtonMash.showButtonMashing && BufferedRate > 30){
            fireMovie()
        }else if(BufferedRate < 33){
    ...
    //  仮死状態になった時に叩き起こすトリガーとして使うかもしれない程度
    useEffect(() => {
        videoLog({before : 'useEffect()[videoRef.ended]', after : videoRef.ended})
    }, [videoRef.ended]);
    //  
    //  再生が終了しているかどうかをBoolean値で表す
    //////////////////////////////////
    ...
    //////////////////////////////////
    //  北斗の終了フラグの返りに反応して再生停止を制御
    //
    useEffect(() => {
        videoLog({before : 'ファイアー❓checkUndercard.isUndercardEnded', after : checkUndercard.isUndercardEnded})
        // 終了済みになっていたら
        if(checkUndercard.isUndercardEnded){
            videoLog({before : 'ファイアー❗️checkUndercard.isUndercardEnded', after : checkUndercard.isUndercardEnded})
            // 非表示にする　レンダリングから撤去
            if(ShowUndercard){setShowUndercard(false)}
            fireMovie('北斗終了のファイアー❗️')
        }else{
            videoLog({before : 'ファイアー❌checkUndercard.isUndercardEnded', after : checkUndercard.isUndercardEnded})
        }
    }, [checkUndercard.isUndercardEnded]);
    //  
    //  北斗の終了フラグの返りに反応して再生停止を制御
    //////////////////////////////////
    ...
    function fireMovie(e) {
        ...
        timeOutFireMovie = setTimeout(() => {
            ...
            .then(() => {                   
                //  まだ出現していなければ前座の表示　
                if(!checkUndercard.isUndercardStarted){
                    ...
                    //  returnに現れる
                    if(!ShowUndercard){setShowUndercard(true)}
                }
    ...
    return (
        <>
            ...
            {/* {checkHokuto.isShowHokuto
                ?<Hokuto data={
                    checkHokuto
                }/>
                :<></>
            } */}
            {checkUndercard.isUndercardAppear
                ?<Hokuto 
                    data={checkUndercard}
                    key="hokuto"
                    />
